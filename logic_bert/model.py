...
class ReasonLayer(nn.Module):
    ...
    def forward(self, input_states):
        ...
        # output of the first hidden layer
        hidden_states = nn.functional.relu(torch.matmul(middle_states, W1) + b1)
        # output of the second layer in mlp
...
class ReasonLayer(nn.Module):
    ...
    def forward(self, input_states):
        ...
        # return middle_states# Skipping the MLP!# jk
        W1, b1, W2, b2 = self.W1, self.b1, self.W2, self.b2
...
class FinalReasonLayer(nn.Module):
    def __init__(self, alpha=50.0):
        super().__init__()
    ...
    def forward(self, input_states):
        ...
        # output of the first hidden layer
        #hidden_states = nn.functional.sigmoig(torch.matmul(middle_states, W1) + b1)
        hidden_states = nn.functional.sigmoid(torch.matmul(middle_states, W1) + b1)
        # output of the second layer in mlp
        output_states = torch.matmul(hidden_states, W2) + b2
        output_states += middle_states
